...
def main():
    ...
    parser.add_argument('--real-data', '-r', default=None,
                       help='真实标注数据路径：labels.json 或 base_data.json；未指定时优先用 labels.json，不存在则用 base_data.json。更新 base_data 后若要看新真实曲线，可显式指定 -r path/to/base_data.json')
    ...
    if not real_data_path:
        # 默认优先 labels.json；若不存在则用 base_data.json（与当前“真实数据”一致）
        default_labels = project_root / "data_process" / "xibeiyuzhicai" / "output" / "labels.json"
        default_base = project_root / "data_process" / "xibeiyuzhicai" / "output" / "base_data.json"
        if default_labels.exists():
            real_data_path = str(default_labels)
            print(f"  📂 真实数据: {real_data_path}")
        elif default_base.exists():
            real_data_path = str(default_base)
            print(f"  📂 真实数据: {real_data_path}（labels.json 不存在，使用 base_data.json）")
        else:
            print("  ⚠️ 未找到默认真实数据文件（labels.json 或 base_data.json）")
    else:
        print(f"  📂 真实数据: {real_data_path}")

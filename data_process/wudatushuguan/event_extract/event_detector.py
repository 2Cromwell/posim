...
# ==================== 可复用接口：从博文列表进行事件检测 ====================
async def detect_events_from_posts(
    posts: List[Dict],
    event_background: str,
    llm_config: Dict = None,
    time_window_hours: float = 1.0,
    max_tag_groups: int = 15,
) -> List[Dict]:
    """
    从博文列表进行事件检测（供波峰检测等场景调用）
    
    Args:
        posts: 博文列表，每条需包含 time(datetime), user_id, username, followers, agent_type,
               verified, content, emotion, reposts, comments, likes, influence_score, url, tags
        event_background: 事件背景描述
        llm_config: LLM配置，默认使用 LLM_CONFIG
        time_window_hours: 标签首次出现后的时间窗口（小时）
        max_tag_groups: 最多处理的标签组数
    
    Returns:
        事件列表 [{"time": ..., "type": ..., "content": ..., ...}, ...]
    """
    if not posts:
        return []
    ...
    if not tagged:
        print("[WARN] 无带标签博文，跳过事件检测")
        return []
    
    # 确保 influence_score 存在
    for p in tagged:
        if 'influence_score' not in p:
            p['influence_score'] = p.get('reposts', 0) * 2 + p.get('comments', 0) * 1.5 + p.get('likes', 0) * 0.5
    
    calculator = TagInfluenceCalculator(tagged)
    tag_stats = calculator.calculate_all()
    ...
    t_min = min(p['time'] for p in tagged)
    t_max = max(p['time'] for p in tagged)
    time_range = f"{t_min.strftime('%Y-%m-%d %H:%M')} ~ {t_max.strftime('%Y-%m-%d %H:%M')}"
    
    llm = LLMManager(llm_config or LLM_CONFIG)
    generator = EventGenerator(event_background, tag_stats, tagged, llm)
    
    selected_tag_groups = await generator.select_tags(top_tags_info, time_range)
    selected_tag_groups = selected_tag_groups[:max_tag_groups]
    
    all_events = []
    for tag_group in selected_tag_groups:
        events = await generator.generate_events_for_tag_group(tag_group)
        all_events.extend(events)

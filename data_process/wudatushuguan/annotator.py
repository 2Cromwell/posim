"""
Wudatushuguan Event Data Annotation System
...
class DataAnnotator:
    def __init__(self, config_path: str = "data_process/wudatushuguan/config.json"):
        ...
        self.event_background = "2023年7月，武汉大学文理学部图书馆自习区，女生杨某指控男生肖某某在图书馆内隔着裤子摩擦生殖器长达一小时并发帖举报（浏览量超10万+），肖某某一方则称其自幼患皮炎湿疹、摩擦系无意识抓痒并提供病历及5名医学专家论证。校方给予肖某某记过处分但未明确定性，称'排除性骚扰'、记过系'应急降温'。2024年6月杨某提起诉讼。2025年7月该事件因新进展再次引发舆论热议。"
        self.stance_target = "武大图书馆事件中各方的行为"
        
        # Time filtering
        self.start_time = datetime.strptime("2025-07-17 00:00:00", "%Y-%m-%d %H:%M:%S")
        self.end_time = datetime.strptime("2025-08-06 06:00:00", "%Y-%m-%d %H:%M:%S")
        
        # Retry and debug settings
        self.max_retries = 3
        self.debug_probability = 0.0005  # 0.05%
        
        self.output_dir = Path("data_process/wudatushuguan/output")
...
{{
  ...
  "emotionality": "情绪性"
    ...
    async def annotate_dataset(self, data_path: str = "data_process/wudatushuguan/output/base_data.json", batch_size: int = 500):
        ...
        print(f"Posts in time range (2025-07-17 to 2025-07-30): {len(time_filtered)}")

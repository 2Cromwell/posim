...
- 个人简介：{user_info.get('description', '无')}"""
    ...
    if pre_posts:
        ...
        behavior_section = f"\n\n该用户在西贝预制菜事件发酵前的历史行为（截至{CUTOFF_TIME.strftime('%Y年%m月%d日')}）：\n{posts_text}"
    else:
        behavior_section = "\n\n该用户在事件发酵前暂无历史行为数据。"
...
3. **事件观点信念（event_opinions）**：如果有相关数据，推断该用户对西贝预制菜事件的初始观点。格式为：时间、主体、观点、原因。如果没有相关数据则返回空列表。
...
{{
    ...
    "event_opinions": [
        {{"time": "2025-07-27T12:00", "subject": "西贝预制菜事件", "opinion": "观点内容", "reason": "原因"}}
    ]
}}"""
...
3. **事件观点信念（event_opinions）**：该用户在西贝预制菜事件初期可能持有的初始观点。考虑其心理特征和行为倾向。
...
{{
    ...
    "psychological_beliefs": ["信念1", "信念2", "信念3"],
    "event_opinions": [
        {{"time": "2025-07-27T14:00", "subject": "西贝预制菜事件", "opinion": "观点内容", "reason": "原因"}}
    ]
}}"""

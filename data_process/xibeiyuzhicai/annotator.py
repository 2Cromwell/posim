...
class DataAnnotator:
    def __init__(self, config_path: str = "data_process/xibeiyuzhicai/config.json"):
        ...
        self.event_background = "2025年9月10日，网红企业家罗永浩在社交媒体上指控西贝餐饮'几乎全都是预制菜'，引发广泛行业风波。罗永浩主张消费者有知情权，呼吁国家推动立法强制餐厅明示是否使用预制菜。西贝创始人贾国龙坚决否认并表示将起诉罗永浩。西贝方面强调其使用的是中央厨房预加工（配送原料）而非传统意义上的预制菜。"
        self.stance_target = "事件中西贝方"
        
        # Time filtering (from config)
        self.start_time = datetime.strptime(config['filter']['start_time'], "%Y-%m-%d %H:%M:%S")
        self.end_time = datetime.strptime(config['filter']['end_time'], "%Y-%m-%d %H:%M:%S")
        
        # Retry and debug settings
        self.max_retries = 3
        self.debug_probability = 0.0005  # 0.05%
        
        self.output_dir = Path("data_process/xibeiyuzhicai/output")
...
{{
  ...
  "emotionality": "情绪性"
    ...
    async def annotate_dataset(self, data_path: str = "data_process/xibeiyuzhicai/output/base_data.json", batch_size: int = 500):
        ...
        print(f"Posts in time range ({self.start_time.strftime('%Y-%m-%d')} to {self.end_time.strftime('%Y-%m-%d')}): {len(time_filtered)}")

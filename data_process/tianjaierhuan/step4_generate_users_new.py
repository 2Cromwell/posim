...
- 个人简介：{user_info.get('description', '无')}"""
    ...
    if pre_posts:
        ...
        behavior_section = f"\n\n该用户在天价耳环事件发酵前的历史行为（截至2025年5月16日）：\n{posts_text}"
    else:
        behavior_section = "\n\n该用户在事件发酵前暂无历史行为数据。"
...
1. **角色身份信念（identity_description）**：用第一人称描述该用户的社交媒体身份定位、行为特点和表达风格。**必须包含其行为动态特征**（如"我通常习惯点赞和转发，很少写长文"）。约80-150字。

2. **心理认知信念（psychological_beliefs）**：该用户在网络舆情参与中可能持有的深层心理认知，体现其对社会、权力、财富等的固有看法。给出3-5条信念条目。
   参考心理类型：自我实现类、猎奇探究类、减压宣泄类、仇官仇富类、跟风从众类。

3. **事件观点信念（event_opinions）**：如果有相关数据，推断该用户对天价耳环事件的初始观点。格式为：时间、主体、观点、原因。如果没有相关数据则返回空列表。
...
{{
    ...
    "event_opinions": [
        {{"time": "2025-05-15T12:00", "subject": "天价耳环事件", "opinion": "观点内容", "reason": "原因"}}
    ]
}}"""
...
1. **角色身份信念（identity_description）**：用第一人称描述该用户可能的社交媒体身份定位、行为特点和表达风格。**必须包含其行为动态特征**（如"我一般就点个赞，很少评论"）。约80-150字。要符合普通微博用户的真实感。

2. **心理认知信念（psychological_beliefs）**：基于抽样信念进行扩展，生成3-5条具体的心理认知条目。要体现真实网民的心理特征。

3. **事件观点信念（event_opinions）**：该用户在天价耳环事件初期（2025年5月10日前后）可能持有的初始观点。考虑其心理特征和行为倾向。
...
{{
    ...
    "psychological_beliefs": ["信念1", "信念2", "信念3"],
    "event_opinions": [
        {{"time": "2025-05-10T14:00", "subject": "天价耳环事件", "opinion": "观点内容", "reason": "原因"}}
    ]
}}"""
